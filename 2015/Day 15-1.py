...
with open('Day 15 - input', 'r') as f:
    Cookie = namedtuple('Cookie', 'name capacity durability flavor texture calories')
    ingredients = []
    for line in f:
        line = line.strip().split()
        ingredients.append(Cookie(name=line[0][:-1], capacity=int(line[2][:-1]), durability=int(line[4][:-1]), flavor=int(line[6][:-1]), texture=int(line[8][:-1]), calories=int(line[10])))
    max_score = 0
    for i in range(101):
