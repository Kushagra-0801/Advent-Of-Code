...
new_power_levels = defaultdict(int)
for size in range(1, 301):
	print('Size %d' % size)
	for x in range(0, 300 - size + 1):
		for y in range(0, 300 - size + 1):
			square_sum = compute_square_sum(x, y, size, matrix)
			new_power_levels[(x, y, size)] = square_sum

coordinate = max(new_power_levels.items(), key=operator.itemgetter(1))[0]
print(coordinate[0] + 1, coordinate[1] + 1, coordinate[2])
